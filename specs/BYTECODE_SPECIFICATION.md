...
- **Код:** `0x94`
- **Операнд:** игнорируется
- **Описание:** Создать новый массив bool на куче
- **Стек:** `..., int_size → ..., array_ref`
- **Примечание:** Размер массива должен быть на стеке. Массив инициализируется значениями `false`.
...
- **Код:** `0xA0`
- **Операнд:** игнорируется
- **Описание:** Печатает примитивное значение на вершине стека в стандартный поток вывода
- **Стек:** `..., value → ...` (значение удаляется со стека)
- **Поддерживаемые типы:**
  - `int` - печатает целое число
  - `float` - печатает число с плавающей точкой
  - `bool` - печатает "true" или "false"
- **Ошибка:** Если на стеке нет значения или значение имеет неподдерживаемый тип, VM падает
- **Примечание:** Эта инструкция генерируется генератором байткода при встрече вызова встроенной функции `print(...)`. Тип значения определяется во время выполнения VM на основе типа значения на стеке.

#### PRINT_ARRAY
- **Код:** `0xA1`
- **Операнд:** игнорируется
- **Описание:** Печатает массив на вершине стека в стандартный поток вывода
- **Стек:** `..., array_ref → ...` (ссылка на массив удаляется со стека)
- **Поддерживаемые типы:**
  - `int[]` - печатает массив в формате `[1, 2, 3, 4, 5]`
  - `float[]` - печатает массив в формате `[1.0, 2.5, 3.14]`
  - `bool[]` - печатает массив в формате `[true, false, true]`
- **Ошибка:** Если на стеке нет значения или значение не является ссылкой на массив, VM падает
- **Примечание:** Эта инструкция генерируется генератором байткода при встрече вызова встроенной функции `printArray(...)`. Массив извлекается из кучи по `heapId` и печатается.
...
### Пример 6: Вызов встроенных функций print и printArray
...
```
PUSH_INT 0      // 42
PRINT           // 0xA0 - печать int
PUSH_FLOAT 0    // 3.14
PRINT           // 0xA0 - печать float
PUSH_BOOL 1     // true
PRINT           // 0xA0 - печать bool
PUSH_INT 1      // 3 (размер массива)
NEW_ARRAY_INT   // Создать массив
STORE_LOCAL 0   // arr
LOAD_LOCAL 0    // arr
PUSH_INT 2      // 0 (индекс)
PUSH_INT 3      // 10 (значение)
ARRAY_STORE     // arr[0] = 10
LOAD_LOCAL 0    // arr
PUSH_INT 2      // 0 (индекс)
PUSH_INT 4      // 20 (значение)
ARRAY_STORE     // arr[1] = 20
LOAD_LOCAL 0    // arr
PUSH_INT 2      // 0 (индекс)
PUSH_INT 5      // 30 (значение)
ARRAY_STORE     // arr[2] = 30
LOAD_LOCAL 0    // arr (ArrayRef на стеке)
...
**Константы:**
```kotlin
intConstants = [42L, 3L, 0L, 10L, 20L, 30L]
floatConstants = [3.14]
```

**Примечание:** Инструкции `PRINT` и `PRINT_ARRAY` генерируются генератором байткода при обнаружении вызовов встроенных функций `print` и `printArray` соответственно. Генератор не добавляет эти функции в список функций модуля, так как это встроенные функции, реализованные на уровне VM.
...
6. **Выполнение встроенных функций:**
   - При выполнении инструкции `PRINT`:
     - Берется примитивное значение с вершины стека операндов
     - В зависимости от типа значения (int, float, bool) выполняется соответствующее форматирование
     - Результат выводится в стандартный поток вывода
     - Значение удаляется из стека
   - При выполнении инструкции `PRINT_ARRAY`:
     - Берется ссылка на массив (ArrayRef) с вершины стека операндов
     - Массив извлекается из кучи по `heapId`
     - Массив печатается в формате `[element1, element2, ...]`
     - Ссылка удаляется из стека
...
- **Неподдерживаемый тип для PRINT** → падение (например, попытка напечатать VoidValue)
- **Недействительная ссылка на массив** → падение (при попытке напечатать ArrayRef с несуществующим heapId)

Семантический анализатор должен гарантировать корректность программы до генерации байткода. Однако для встроенных функций типа `print` проверка типов выполняется на этапе семантического анализа, поэтому VM может полагаться на корректность типов значений на стеке.
