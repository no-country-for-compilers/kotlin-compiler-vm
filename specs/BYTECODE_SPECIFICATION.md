...
- **Код:** `0xA0`
- **Операнд:** игнорируется
- **Описание:** Печатает значение на вершине стека в стандартный поток вывода
...
### Пример 6: Вызов встроенной функции print
...
**Примечание:** Инструкция `PRINT` генерируется генератором байткода при обнаружении вызова встроенной функции `print`. Генератор не добавляет `print` в список функций модуля, так как это встроенная функция, реализованная на уровне VM.
...
6. **Выполнение встроенных функций:**
   - При выполнении инструкции `PRINT`:
     - Берется значение с вершины стека операндов
     - В зависимости от типа значения (int, float, bool, ArrayRef) выполняется соответствующее форматирование
     - Результат выводится в стандартный поток вывода
     - Значение удаляется из стека
   - Для массивов: массив извлекается из кучи по `heapId` и печатается в формате `[element1, element2, ...]`
