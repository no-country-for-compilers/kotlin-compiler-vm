...
- Таблицу функций (`functions: Map<String, List<FunctionSymbol>>`) - поддерживает перегрузку функций
- Ссылку на родительскую область (`parent: Scope?`)

**Примечание:** Таблица функций хранит список функций для каждого имени, что позволяет поддерживать перегрузку функций (несколько функций с одним именем, но разными типами параметров).
...
Добавляет функцию в текущую область. Поддерживает перегрузку - несколько функций с одним именем, но разными типами параметров. Возвращает `false`, если функция с такой же сигнатурой уже определена в этой области.
...
Ищет функцию по имени, начиная с текущей области и поднимаясь по цепочке родительских областей. Возвращает первую найденную функцию с данным именем (для обратной совместимости).

#### resolveFunction(name: String, argTypes: List<Type>): FunctionSymbol?
Ищет функцию по имени и типам аргументов (для поддержки перегрузки). Возвращает функцию, параметры которой совместимы с переданными типами аргументов, или `null`, если подходящая функция не найдена.
...
Функции с одинаковым именем и одинаковыми типами параметров не допускаются:
...
**Перегрузка функций:** Функции с одинаковым именем, но разными типами параметров разрешены (перегрузка):

```kotlin
// ✅ Корректно - перегрузка функций
func foo(x: int): void { }
func foo(x: float): void { }  // Разрешено, так как типы параметров разные
```
...
Функция `print` доступна в глобальной области видимости и имеет следующие перегрузки:

```kotlin
// Печать простых типов
