...
Требования:
- повторный `free` или `free` несуществующего id считается ошибкой выполнения.
...
### 5.4 Вложенные ссылки (будущее расширение)
Если в будущем появятся объекты, содержащие ссылки на другие heap-объекты, то при `free` необходимо рекурсивно `release` дочерние ссылки.
В текущей версии массивы содержат только примитивы — рекурсия не нужна.
...
## 11. Тестируемость и debug-интерфейс (для unit-тестов)

Рекомендуется иметь (только для тестов/отладки):
- `debugRefCount(ArrayRef): Int`
- `debugHeapObjectCount(): Int`

---

## 12. Минимальные критерии корректности

1) Создание массива через `NEW_ARRAY_INT/NEW_ARRAY_FLOAT` даёт `ArrayRef` с `refCount=1`.
2) `LOAD_LOCAL` увеличивает `refCount` (если ссылка).
3) `STORE_LOCAL` освобождает старое значение в слоте (если ссылка).
4) `POP` уменьшает `refCount` (если ссылка) и освобождает объект при достижении 0.
5) После очистки всех root owners (stack/locals) heap пуст (нет утечек).
